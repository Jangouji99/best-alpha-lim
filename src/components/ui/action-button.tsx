...
const ActionsButton: React.FC<{ id: string; lang: string; }> = ({ id, lang }) => {
  const router = useRouter();
  const { openDrawer, setDrawerView } = useUI();
  const { data, isLoading, error } = useGetQuotationDetails(
    id,
    "en"
  );

  function handleCartOpen() {
    router.push(`/${lang}/complete-order/${id}`);
    // setDrawerView('ORDER_DETAILS');
    // return openDrawer(items);
  }
